...
func _on_detection_area_body_entered(body) -> void:
	if body.is_in_group("player"):
		_player_ref = body
		randomMoveTimer.stop()  # Para o movimento aleatório quando o player é detectado

# Função chamada quando o jogador sai da área de detecção
func _on_detection_area_body_exited(body) -> void:
	if body == _player_ref:
		_player_ref = null
		# Verifica se o timer é válido e está na árvore antes de iniciar
		if is_instance_valid(randomMoveTimer) and randomMoveTimer.is_inside_tree():
			randomMoveTimer.start()  # Reinicia o movimento aleatório quando o player sai
