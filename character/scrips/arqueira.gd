class_name Arqueira extends CharacterBody2D
...
const flecha := preload("res://projeteis/flechas/normal/Flecha.tscn")
...
func take_damage() -> void:
	...
	if Global.health <= 0:
		...
		dano_sfx.play()  # Reproduz som de dano
	healthChanged.emit(Global.health)
	# Atualiza a barra de vida
	if is_instance_valid(healthbar):
		# Atualiza o valor máximo da barra de vida
...
func _attack() -> void:
	if Input.is_action_just_pressed("attack") and not _is_attacking:
		if Global.mana > 0:  # Verifica se há mana suficiente para atacar
			Global.mana -= 0.4  # Deduz 1 ponto de mana
		...
		attack_sfx.play()  # Reproduz som de ataque
...
func _on_hurt_box_area_entered(area):
	if area.has_method("collect"):
		...
		coletarItems_sfx.play()  # Reproduz som ao coletar item
...
func use_item(item: InventoryItem) -> void:
	if item is HealthItem and Global.health < Global.maxHealth + (StatusPlayer.Vitalidade * 1.5):
		...
		usarPocao_sfx.play()  # Reproduz som de uso de poção
