class_name Feiticeira extends CharacterBody2D
...
@onready var ice_sfx : AudioStreamPlayer = $ice_sfx
@onready var fire_sfx : AudioStreamPlayer = $fire_sfx
...
func _ready() -> void:
	...
	manabar.init_mana(Global.mana, Global.maxMana)
...
func _attack() -> void:
	if Input.is_action_just_pressed("attack") and not _is_attacking:
		manaChanged.emit(Global.mana)  # Emite o sinal para atualizar a interface
		...
		attack_sfx.play()
		_attack_timer.start()
	if Input.is_action_just_pressed("ice") and not _is_attacking:
		if Global.mana > 0:  # Verifica se há mana suficiente para atacar
			Global.mana -= 1  # Deduz 1 ponto de mana
			manaChanged.emit(Global.mana)  # Emite o sinal para atualizar a interface
			...
			# Desabilitar a Hitbox ao iniciar o ataque
			if attack_hitbox:
			...
			print("Mana insuficiente para atacar!")
	if Input.is_action_just_pressed("fire") and not _is_attacking:
		if Global.mana > 0:  # Verifica se há mana suficiente para atacar
			Global.mana -= 1  # Deduz 1 ponto de mana
			manaChanged.emit(Global.mana)  # Emite o sinal para atualizar a interface
...
func _on_attack_timer_timeout() -> void:
	...
	# Reabilitar a Hitbox ao terminar o ataque
	if attack_hitbox:
...
func projeteis(tipo: String) -> void:
	var instancia
	...
	instancia.set_direction(attack_dir)  # Usa a direção de ataque.
...
func increase_mana(amount: int) -> void:
	Global.mana += amount
	Global.mana = min(Global.maxMana + (StatusPlayer.Inteligencia * 1.5), Global.mana)
	manaChanged.emit(Global.mana)
	# Atualiza a barra de mana
	if is_instance_valid(manabar):
		# Atualiza o valor máximo da barra de mana
...
func calculate_mana() -> float:
	...
	return Global.maxMana + (status.Inteligencia * 1.5)
