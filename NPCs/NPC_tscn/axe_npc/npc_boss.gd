...
func _physics_process(_delta: float) -> void:
	...
	if(Input.is_action_just_pressed("interagir") && colidiu[1]):
		if(get_node("/root").get_child(11).find_child("UI").get_child(11)==null):
			...
			caixa_dialogo.text.text="Então você chegou até aqui."
			caixa_dialogo.add_array_message(
			[
			"O chefe de todos os monstros está presente aqui.",
			"Acha que consegue derrotar essa criatura?!",
			"Incrivel, é dessa confiança que estavamos precisando!!"
			])
